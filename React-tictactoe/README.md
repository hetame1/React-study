...
- React는 먼저 컴포넌트를 렌더링하고 그 다음에 결과물을 DOM에 반영한다

- 이 과정에서 만약 컴포넌트가 React.memo()로 둘러 쌓여 있다면 컴포넌트를 렌더링하고 결과를 메모이징(Memoizing)한다

- 그리고 다음 렌더링이 일어날 때 렌더링하는 컴포넌트의 props가 같다면 React는 메모이징(Memoizing)된 내용을 재사용한다

- props 를 비교할 때는 얕은(shallow) 비교를 하기 때문에 객체의 변화를 감지하지 못한다

- 비교 방식을 수정하고 싶을때는 두번째 파라미터로 비교 함수를 넣어주면 된다

## 얕은 비교(Shallow Comparison)

- 숫자, 문자열 등 원시 자료형은 값을 비교

- 배열, 객체 등 참조 자료형은 값, 속성을 비교하는게 아니라 참조되는 위치(주소값)를 비교
...
2.  Object depth가 깊은 경우: lodash의 isEqual() 사용

## useCallback

- 원래 컴포넌트가 렌더링 될 때 그 안에 있는 함수도 다시 만들게 됩니다

- 이렇게 되면 자식 컴포넌트에 props로 전달해주는 함수들은 렌더링 될 때마다 새로 만들어진 함수를 전달하게 된다

- 이러한 상황을 방지하기 위해서 useCallback을 사용한다

- 사용방법은 useCallback(생성하고 싶은 함수, [바뀌는 값]) 이런식으로 사용한다

## useMemo

- 함수에 넘겨주는 값이 이전과 동일하다면 리 렌더링 되더라도 연산을 하지않고 이전에 연산한 결과를 재사용한다
